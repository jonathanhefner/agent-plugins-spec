...
The `web.fetch` profile applies to a non-MCP tool whose defined operation is to retrieve content from one or more absolute HTTP or HTTPS URLs and make that content, or a derived representation of it, available to the agent. For this profile, `tool_profile.name` MUST be `"web.fetch"`. `tool_profile.input` contains exactly one field:
...
- [ ] When a pending call is determined to match the `mcp`, `shell`, `file.read`, `file.modify`, or `web.fetch` profile, add the corresponding `tool_profile` while preserving the client-native tool name and input; otherwise, omit `tool_profile` ([§7.3.4](#734-toolbefore))
