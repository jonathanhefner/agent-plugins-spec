...
- [ ] When a pending call is determined to match the `mcp`, `shell`, `file.read`, or `web.fetch` profile, add the corresponding `tool_profile` while preserving the client-native tool name and input; otherwise, omit `tool_profile` ([§7.3.4](#734-toolbefore))
