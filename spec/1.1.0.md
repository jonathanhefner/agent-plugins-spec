...
The fixed discovery location is `skills/`. Starting with each immediate child directory of `skills/`, clients MUST recursively inspect directories as follows:

1. If a directory contains an immediate entry named exactly `SKILL.md`, the directory is a terminal skill candidate. The client MUST NOT inspect its descendants for additional skills. This boundary applies before the client determines whether the `SKILL.md` entry resolves to a regular file within the plugin root or whether the skill conforms to the Agent Skills specification.
2. Otherwise, the directory is a skill namespace directory. Its name MUST be 1–64 characters, contain only lowercase ASCII letters (`a-z`), digits (`0-9`), and hyphens (`-`), begin and end with a letter or digit, and contain no consecutive hyphens. If the name is invalid, the client MUST skip that directory and its descendants, and SHOULD report the invalid namespace.
3. For each valid skill namespace directory, the client MUST apply this procedure to its child directories. Other entries have no skill-discovery semantics.

While discovering skills, clients MUST NOT follow symbolic links, junctions, reparse points, or equivalent filesystem links that resolve to directories. This restriction applies only to skill discovery and does not otherwise prohibit links that resolve within the plugin root under §4.1.

Clients MAY stop skill discovery when a client-defined resource limit is exceeded, such as a limit on directory entries inspected. Clients SHOULD report when such a limit prevents complete discovery.

A terminal skill candidate is a skill only if its `SKILL.md` resolves to a regular file within the plugin root and the skill conforms to the Agent Skills specification. Otherwise, the client MUST skip that candidate and SHOULD report the invalid skill. Skipping a terminal candidate does not permit the client to inspect its descendants for additional skills.

If a client also supports Agent Plugins 1.0.0, it MUST apply the 1.0.0 skill-discovery rules to plugins that target 1.0.0. Namely, it MUST inspect only immediate child directories of `skills/` and MUST NOT inspect deeper descendants for skills.

#### Identity and client mappings

Each skill directory at a distinct path relative to `skills/` represents a distinct skill, including when multiple skills declare the same Agent Skills `name`. How a client identifies, presents, or exposes discovered skills is implementation-defined. A client MUST NOT merge distinct skills or silently select one when its client-specific mapping collides. If the client cannot preserve the distinction among a set of skills, it MUST skip every skill in that collision set, SHOULD report the collision, and MUST continue loading other skills and component types.

Client-specific mappings may be lossy. For example, joining path segments with hyphens maps both `a-b/c/deploy` and `a/b-c/deploy` to `a-b-c-deploy`. Clients determine whether and how to avoid or disambiguate such collisions.

#### Examples

Example: three distinct skill directories that all contain `name: deploy`:
...
The relative paths `deploy`, `aws/deploy`, and `gcp/deploy` represent three distinct skills.

Example: a `SKILL.md` within an existing skill is not discovered as another skill:
...
The client treats `skill-creator` as a terminal skill candidate and does not inspect `templates/` for additional skills.
...
- [ ] Recursively discover skills through valid namespace directories without following filesystem links to directories ([§7.1](#71-skills))
- [ ] Treat an immediate `SKILL.md` entry as a terminal discovery boundary before validating the skill ([§7.1](#71-skills))
- [ ] Preserve distinct skill paths and isolate any unresolved client-specific collision set ([§7.1](#71-skills))
...
### Why is skill discovery tree-shaped?

Namespaced skill discovery stops at the first `SKILL.md` entry on each branch before validating the skill, so files bundled within a skill cannot be reinterpreted as nested skills. It does not follow directory links because links can turn the hierarchy into a graph with cycles or multiple namespace paths to the same directory. Together, these rules keep discovery finite and deterministic. The specification does not impose a depth or breadth limit because either would restrict one tree shape without bounding traversal work generally; clients may instead apply resource limits appropriate to their implementations.
