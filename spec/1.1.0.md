...
| Field             | Type       | Description                                                                 |
| ----------------- | ---------- | --------------------------------------------------------------------------- |
| `hook_event_name` | string     | Always `"tool.before"`.                                                    |
| `session_id`      | string     | Opaque identifier for the root agent session.                               |
| `cwd`             | string     | Absolute client working directory for the agent when the event is emitted.  |
| `tool_use_id`     | string     | Opaque identifier for the pending tool call, unique within the session.      |
| `tool_name`       | string     | Client-native tool name in the pending call.                                 |
| `tool_input`      | JSON value | Client-native tool input that would be used if the call proceeds unchanged. |

`tool_name` and `tool_input` are intentionally opaque. Agent Plugins does not normalize built-in tool identities or argument schemas.

The client defines session boundaries. It MUST use the same `session_id` for hook invocations it associates with the same root agent session. Handlers MUST otherwise treat `session_id` as opaque. The `cwd` field does not imply that relative paths within the opaque `tool_input` resolve against it.
...
### Why are tool names and inputs opaque?

Clients do not share built-in tool names or argument schemas. Preserving the native JSON lets portable hooks inspect tools they recognize without pretending that unlike tools have a common data model. The hooks configuration does not include tool-name matchers because a handler can inspect the same input itself, avoiding a second matching language and client-specific naming assumptions.
